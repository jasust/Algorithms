...
int main()
{
    ...
    for (i=0; i<min(sizer, (int)res.size()); i++) { fscanf(g,"%d", &nzm); if (nzm-res[i]) neradi = true; }
    for (i=0; i<(sizer-(int)res.size()); i++) { fscanf(g,"%d", &nzm); if (nzm) neradi = true; }
